...
const empty = { name: '', email: '', grade: '', phone: '', enrolled: '', status: 'active' };
...
export default function Students() {
  const { students, add, update, remove } = useSchool();
  ...
  const filtered = students.filter(
    (s) => s.name.toLowerCase().includes(search.toLowerCase()) || s.email.toLowerCase().includes(search.toLowerCase()) || s.grade.includes(search)
  );
  ...
  return (
    <div className="space-y-6">
          ...
          <tbody>
            {filtered.map((s) => (
              <tr key={s.id} className="border-b border-school-border/50 hover:bg-white/5">
                ...
                <td className="px-4 py-3 text-slate-300">{s.enrolled}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${s.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                    {s.status}
                  </span>
                </td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => openEdit(s)} className="text-school-primary hover:text-school-primaryLight text-xs mr-3">Edit</button>
                  <button onClick={() => remove(s.id)} className="text-school-danger hover:text-red-400 text-xs">Delete</button>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editId ? 'Edit Student' : 'Add Student'}>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input value={form.grade} onChange={(e) => set('grade', e.target.value)} placeholder="Grade (e.g. 10)" className="bg-white/5 border border-school-border rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-school-primary" />
          ...
          <div className="grid grid-cols-2 gap-3">
            <input value={form.enrolled} onChange={(e) => set('enrolled', e.target.value)} type="date" className="bg-white/5 border border-school-border rounded-lg px-4 py-2.5 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-school-primary" />
